...
import {COMMISSION_CARD_MANIFEST} from './cards/commission/CommissionCardManifest';
...
export class GameCards {
  ...
  public constructor(gameOptions: GameOptions) {
    ...
    const manifests: Array<[boolean, ModuleManifest]> = [
      ...
      [gameOptions.commissionCardsOption, COMMISSION_CARD_MANIFEST],
  ...
  public static isCorpCompatibleWith(cardName:CardName, gameOptions:GameOptions): ICorporationCard | undefined {
    let cf;
    for (const moduleManifest of ALL_MODULE_MANIFESTS) {
      const cardManifest = moduleManifest['corporationCards'];
      cf = cardManifest[cardName];
      if (cf !== undefined) {
        break;
      }
    }
