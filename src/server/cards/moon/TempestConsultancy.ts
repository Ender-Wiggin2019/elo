...
export class TempestConsultancy extends CorporationCard implements ICorporationCard {
  ...
  public canAct(player: IPlayer) {
    return player.tags.count(Tag.MOON) >= 5 && TurmoilUtil.getTurmoil(player.game).getAvailableDelegateCount(player) > 0;
  }

  public action(player: IPlayer) {
    let count = Math.floor(player.tags.count(Tag.MOON) / 5);
    count = Math.min(count, 3);
    count = Math.min(count, TurmoilUtil.getTurmoil(player.game).getAvailableDelegateCount(player));
    if (count > 0) {
