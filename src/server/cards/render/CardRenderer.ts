...
abstract class Builder<T> {
  ...
  public colonyPlacementBonus(): Builder<T> {
    this._appendToRow(new CardRenderItem(CardRenderItemType.COLONY_PLACEMENT_BONUS));
    return this;
  }
  ...
  public noTags(options?: ItemOptions) {
    return this._appendToRow(new CardRenderItem(CardRenderItemType.NO_TAGS, -1, options));
  }
  ...
  public blankTag(amount: number = 1) {
  ...
  /*
   * 每一个CardRenderEffect都包含三行，通过startEffect或者startAction换行，
   * 即每个 Builder.effect或者Builder.action方法的eb回调里面都必须包含恰好一个startEffect或者startAction。
   * 也可以不写startEffect或者startAction， 手动.br换行，使得最终含有3行，但是第二行必须为 CardRenderSymbol 类型
   *
   * 前端渲染的时候  通过CardRenderEffectBoxComponent 获取 delimiter 属性，如果第一行为空，则不显示第二行，
   * 即不显示效果的 ":" 或者 行动的 "->" 符号
   */
