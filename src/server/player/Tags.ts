...
export class Tags {
  ...
  public count(tag: Tag, mode: CountingMode = 'default') {
    const includeEvents = mode === 'raw-underworld' || this.player.tableau.has(CardName.ODYSSEY) || this.player.tableau.has(CardName._INTERPLANETARY_CINEMATICS_);
  ...
  public distinctCount(mode: DistinctCountMode, extraTag?: Tag): number {
    ...
    const interplanetary = this.player.tableau.has(CardName._INTERPLANETARY_CINEMATICS_);
    let wildTagCount = 0;
    ...
    for (const card of this.player.tableau) {
      ...
      if (playerIsOdyssey || interplanetary || card.type !== CardType.EVENT) {
        for (const tag of card.tags) {
          if (tag === Tag.WILD) {
            wildTagCount++;
          } else {
