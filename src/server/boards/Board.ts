...
export abstract class Board {
  ...
  public static deserializeSpace(serialized: SerializedSpace, players: ReadonlyArray<IPlayer>): Space {
    ...
    if (serialized.player) {
      player = this.findPlayer(players, serialized.player?.id);
      // Prevent loss of "neutral" player tile ownership across reloads
      if (serialized.player !== undefined && player === undefined) {
        console.error('deserializeSpace error ', serialized, serialized.player);
      }
    }
    ...
    const space: Space = {
      ...
      adjacency: serialized.adjacency,
      x: serialized.x,
      y: serialized.y,
    };
  ...
  public static deserialize(board: SerializedBoard, players: ReadonlyArray<IPlayer>): {spaces: Array<Space>} {
    ...
    if (spaces.find((space) => space.id === SpaceName.TRANTOR) === undefined) {
      spaces.push(colonySpace(SpaceName.TRANTOR));
    }
    return {spaces};
  }
}
