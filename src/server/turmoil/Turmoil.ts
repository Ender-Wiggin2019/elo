...
export class Turmoil {
  ...
  public sendDelegateToParty(delegate: Delegate, partyName: PartyName, game: IGame, throwIfError = false): void {
    ...
    // 如果是西斯，所有放置的代表都是中立代表
    /**
    const sith = game.players.find((p) => p.isCorporation(CardName.SITH_ORGANIZATIONS));
  ...
  public endGeneration(game: IGame): void {
    ...
    game.players.forEach((player) => {
      if (player.playedCards.has(CardName.POLITICALREFORM)) {
  ...
  public onAgendaSelected(game: IGame): void {
    ...
    if (this.chairman !== undefined && this.chairman !== 'NEUTRAL' && game.getPlayerById(this.chairman).isCorporation(CardName.SITH_ORGANIZATIONS)) {
      const NonSithPlayers = game.players.filter((p) => !p.isCorporation(CardName.SITH_ORGANIZATIONS));
  ...
  public getInfluence(player: IPlayer) {
    ...
    if (player.playedCards.has(CardName.INCITE_ENDER)) {
  ...
  public getSendDelegateInput(player: IPlayer): SelectParty | undefined {
    if (this.hasDelegatesInReserve(player)) {
      ...
      } else if (player.playedCards.has(CardName.INCITE) && player.canAfford(3)) {
