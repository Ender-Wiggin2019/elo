...
describeDatabaseSuite({
  ...
  otherTests: (dbFactory: () => TestPostgreSQL) => {
    ...
    it('getGames - returns in order of last saved', async () => {
      ...
      expect((await db.getGames()).map((data) => data.gameId)).deep.eq(['game-id-1111', 'game-id-3333', 'game-id-2222']);

      game2.save();
      await db.lastSaveGamePromise;

      expect((await db.getGames()).map((data) => data.gameId)).deep.eq(['game-id-2222', 'game-id-1111', 'game-id-3333']);
    ...
    it('undo works in multiplayer, other players have passed', async () => {
      ...
      const lastSaveId = game.lastSaveId - 1;
      await Database.getInstance().restoreGame(player.game.id, lastSaveId, game, 'pp');
      await db.awaitAllSaves();
      const revisedPlayer = game.getPlayerById(player.id);
    ...
    it('undo works in solo', async () => {
      ...
      // This is embedded in routes/PlayerInput, and should be moved out of there.
      const lastSaveId = game.lastSaveId - 1;
      expect(lastSaveId).eq(5);
      expect(game.lastSaveId).eq(7);
      await Database.getInstance().restoreGame(player.game.id, lastSaveId, game, '');
      await db.awaitAllSaves();
      const revisedPlayer = game.getPlayerById(player.id);
      expect(revisedPlayer.megaCredits).eq(4);
      expect(revisedPlayer.actionsTakenThisRound).eq(4);
